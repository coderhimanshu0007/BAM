...
public class CollectionOSAgeingRequester implements BaseRequester {

    public CollectionOSAgeingRequester() {
    }

    @Override
    public void run() {
        ApiResponse<Object> apiResponse = HTTPOperationController.collectionOSAgeing();
        if (apiResponse != null) {
            if (apiResponse.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                if (apiResponse.getResponse() != null) {
                    EventBus.getDefault().post(new EventObject(Events.GET_COLLECTION_OS_AGEING_SUCCESSFULL, apiResponse.getResponse()));
                } else {
                    EventBus.getDefault().post(new EventObject(Events.GET_COLLECTION_OS_AGEING_UNSUCCESSFULL, null));
                }
            } else {
