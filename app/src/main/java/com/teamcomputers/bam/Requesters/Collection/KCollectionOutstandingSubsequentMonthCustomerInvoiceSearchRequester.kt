...
class KCollectionOutstandingSubsequentMonthCustomerInvoiceSearchRequester(var customer: String, var invoice: String) : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().collectionOutstandingSubsequentMonthCustomerInvoiceSearch(customer, invoice)
        if (apiResponse != null) {
