...
public class PurchaseSalesOrderRequester implements BaseRequester {

    public PurchaseSalesOrderRequester() {
    }

    @Override
    public void run() {
        ApiResponse<Object> apiResponse = HTTPOperationController.purchaseSalesOrder();
        if (apiResponse != null) {
            if (apiResponse.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                if (apiResponse.getResponse() != null) {
                    EventBus.getDefault().post(new EventObject(Events.GET_PURCHASE_SALES_ORDER_SUCCESSFULL, apiResponse.getResponse()));
                } else {
                    EventBus.getDefault().post(new EventObject(Events.GET_PURCHASE_SALES_ORDER_UNSUCCESSFULL, null));
                }
            } else {
