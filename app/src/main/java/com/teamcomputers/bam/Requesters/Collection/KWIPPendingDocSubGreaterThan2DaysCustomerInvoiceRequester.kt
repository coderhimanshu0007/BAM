...
class KWIPPendingDocSubGreaterThan2DaysCustomerInvoiceRequester(var customer: String, var start: String, var end: String, var loadMore: String) : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().WIPPendingDocSubGreaterThan2DaysCustomerInvoice(customer, start, end)
        if (apiResponse != null) {
