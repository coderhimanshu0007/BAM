    ...
    override fun run() {
        ...
        if (apiResponse != null) {
            if (apiResponse.responseCode == HttpURLConnection.HTTP_OK) {
                ...
                EventBus.getDefault().post(EventObject(BAMConstant.Events.OOPS_MESSAGE, null))
            }
        } else {
            EventBus.getDefault().post(EventObject(BAMConstant.Events.NO_INTERNET_CONNECTION, null))
        }
    }
}