...
public class InstallationHoldRequester implements BaseRequester {

    public InstallationHoldRequester() {
    }

    @Override
    public void run() {
        ApiResponse<Object> apiResponse = HTTPOperationController.installationHold();
        if (apiResponse != null) {
            if (apiResponse.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                if (apiResponse.getResponse() != null) {
                    EventBus.getDefault().post(new EventObject(Events.GET_INSTALLATION_HOLD_SUCCESSFULL, apiResponse.getResponse()));
                } else {
                    EventBus.getDefault().post(new EventObject(Events.GET_INSTALLATION_HOLD_UNSUCCESSFULL, null));
                }
            } else {
