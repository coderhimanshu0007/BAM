...
class KCollectionOutstandingSubsequentMonthInvoiceRequester(var customer: String, var start: String, var end: String) : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().collectionOutstandingSubsequentMonthInvoice(customer, start, end)
        if (apiResponse != null) {
