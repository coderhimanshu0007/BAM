...
class KSalesOpenOrderSearchRequester(val userId: String, val level: String, val documnetNo: String) : KBaseRequester {

    override fun run() {
        val apiResponse = KHTTPOperationController().salesOpenOrderSearch(userId, level, documnetNo)
        if (apiResponse != null) {
            if (apiResponse.responseCode == HttpURLConnection.HTTP_OK) {
                if (apiResponse.response != null) {
                    EventBus.getDefault().post(EventObject(KBAMConstant.Events.GET_SO_SERACH_SUCCESSFULL, apiResponse.response))
                } else {
                    EventBus.getDefault().post(EventObject(KBAMConstant.Events.GET_SO_SERACH_UNSUCCESSFULL, null))
                }
