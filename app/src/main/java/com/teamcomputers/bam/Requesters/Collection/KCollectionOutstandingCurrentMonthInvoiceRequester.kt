...
class KCollectionOutstandingCurrentMonthInvoiceRequester(var customer: String, var start: String, var end: String, var loadMore: String) : BaseRequester {
    override fun run() {
        ...
        if (apiResponse != null) {
            if (apiResponse.responseCode == HttpURLConnection.HTTP_OK) {
                if (apiResponse.response != null) {
                    ...
                    if (loadMore.equals("0")) {
                        EventBus.getDefault().post(EventObject(KBAMConstant.Events.GET_TOTAL_OUTSTANDING_INVOICE_SUCCESSFULL, null))
                    } else if (loadMore.equals("1")) {
