...
class KCollectionPCMInvoiceSearchRequester(var customer: String, var invoice: String) : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().collectionPCMInvoiceSearch(customer, invoice)
        if (apiResponse != null) {
