...
class KCollectionCollectibleOutStandingCustomerInvoiceSearchRequester(var customer: String, var invoice: String) : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().collectionCollectibleOutStandingCustomerInvoiceSearch(customer, invoice)
        if (apiResponse != null) {
