...
class KSOLoadMoreRequester(var userId: String, var level: String, var type: String, var RSM: String, var sales: String, var customer: String, var stateCode: String, var product: String, var documnetNo: String, var startIndex: String, var endIndex: String, var minAmount: String, var maxAmount: String, var minNOD: String, var maxNOD: String) : KBaseRequester {
        ...
        if (apiResponse != null) {
            if (apiResponse.responseCode == HttpURLConnection.HTTP_OK) {
                ...
                if (apiResponse.response != null) {
                    EventBus.getDefault().post(EventObject(KBAMConstant.Events.GET_SO_LOAD_MORE_SUCCESSFULL, apiResponse.response))
                } else {
                    EventBus.getDefault().post(EventObject(KBAMConstant.Events.GET_SO_LOAD_MORE_UNSUCCESSFULL, null))
                }
