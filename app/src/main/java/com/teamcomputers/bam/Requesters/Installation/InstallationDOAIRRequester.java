...
public class InstallationDOAIRRequester implements BaseRequester {

    public InstallationDOAIRRequester() {
    }

    @Override
    public void run() {
        ApiResponse<Object> apiResponse = HTTPOperationController.installationDOAIR();
        if (apiResponse != null) {
            if (apiResponse.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                if (apiResponse.getResponse() != null) {
                    EventBus.getDefault().post(new EventObject(Events.GET_INSTALLATION_DOA_IR_SUCCESSFULL, apiResponse.getResponse()));
                } else {
                    EventBus.getDefault().post(new EventObject(Events.GET_INSTALLATION_DOA_IR_UNSUCCESSFULL, null));
                }
            } else {
