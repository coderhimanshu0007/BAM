...
class KCollectionCollectibleOutStandingCurrentMonthCustomerInvoiceSearchRequester(var customer: String, var invoice: String) : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().collectionCollectibleOutStandingCurrentMonthCustomerInvoiceSearch(customer, invoice)
        if (apiResponse != null) {
            if (apiResponse.responseCode == HttpURLConnection.HTTP_OK) {
                if (apiResponse.response != null) {
                    EventBus.getDefault().post(EventObject(KBAMConstant.Events.GET_TOTAL_OUTSTANDING_INVOICE_SEARCH_SUCCESSFULL, apiResponse.response))
                } else {
                    EventBus.getDefault().post(EventObject(KBAMConstant.Events.GET_TOTAL_OUTSTANDING_INVOICE_SEARCH_UNSUCCESSFULL, null))
                }
