...
class KWIPPendingDocSubLessThan2DaysCustomerInvoiceSearchRequester(var customer: String, var invoice: String) : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().WIPPendingDocSubLessThan2DaysCustomerInvoiceSearch(customer, invoice)
        if (apiResponse != null) {
