...
public class CollectionCollectionRequester implements BaseRequester {

    public CollectionCollectionRequester() {
    }

    @Override
    public void run() {
        ApiResponse<Object> apiResponse = HTTPOperationController.collectionCollection();
        if (apiResponse != null) {
            if (apiResponse.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                if (apiResponse.getResponse() != null) {
                    EventBus.getDefault().post(new EventObject(Events.GET_COLLECTION_COLLECTION_SUCCESSFULL, apiResponse.getResponse()));
                } else {
                    EventBus.getDefault().post(new EventObject(Events.GET_COLLECTION_COLLECTION_UNSUCCESSFULL, null));
                }
            } else {
