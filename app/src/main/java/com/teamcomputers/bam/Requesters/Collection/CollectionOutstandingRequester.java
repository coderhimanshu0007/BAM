...
public class CollectionOutstandingRequester implements BaseRequester {

    public CollectionOutstandingRequester() {
    }

    @Override
    public void run() {
        ApiResponse<Object> apiResponse = HTTPOperationController.collectionOutstanding();
        if (apiResponse != null) {
            if (apiResponse.getResponseCode() == HttpURLConnection.HTTP_OK) {
                ...
                if (apiResponse.getResponse() != null) {
                    EventBus.getDefault().post(new EventObject(Events.GET_COLLECTION_OUTSTANDING_SUCCESSFULL, apiResponse.getResponse()));
                } else {
                    EventBus.getDefault().post(new EventObject(Events.GET_COLLECTION_OUTSTANDING_UNSUCCESSFULL, null));
                }
            } else {
