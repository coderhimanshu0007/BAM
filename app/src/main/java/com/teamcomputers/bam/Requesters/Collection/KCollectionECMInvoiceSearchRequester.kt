...
class KCollectionECMInvoiceSearchRequester(var customer: String, var invoice: String) : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().collectionECMInvoiceSearch(customer, invoice)
        if (apiResponse != null) {
