...
class KCollectionCollectionRequester : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().collectionCollection()
        if (apiResponse != null) {
            if (apiResponse.responseCode == HttpURLConnection.HTTP_OK) {
                if (apiResponse.response != null) {
                    EventBus.getDefault().post(EventObject(KBAMConstant.Events.GET_COLLECTION_COLLECTION_SUCCESSFULL, apiResponse.response))
                } else {
