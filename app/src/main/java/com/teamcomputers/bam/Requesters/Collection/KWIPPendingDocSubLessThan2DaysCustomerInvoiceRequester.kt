...
class KWIPPendingDocSubLessThan2DaysCustomerInvoiceRequester(var customer: String, var start: String, var end: String, var loadMore: String) : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().WIPPendingDocSubLessThan2DaysCustomerInvoice(customer, start, end)
        if (apiResponse != null) {
