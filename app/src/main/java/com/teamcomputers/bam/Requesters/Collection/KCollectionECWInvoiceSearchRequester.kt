...
class KCollectionECWInvoiceSearchRequester(var customer: String, var invoice: String) : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().collectionECWInvoiceSearch(customer, invoice)
        if (apiResponse != null) {
