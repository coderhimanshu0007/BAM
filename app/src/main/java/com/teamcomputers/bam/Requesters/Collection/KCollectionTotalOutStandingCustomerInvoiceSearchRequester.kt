...
class KCollectionTotalOutStandingCustomerInvoiceSearchRequester(var customer: String, var invoice: String) : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().collectionTotalOutstandingCustomerInvoiceSearch(customer, invoice)
        if (apiResponse != null) {
