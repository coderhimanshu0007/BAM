...
class KWIPPendingDocSubGreaterThan2DaysCustomerInvoiceSearchRequester(var customer: String, var invoice: String) : BaseRequester {
    override fun run() {
        val apiResponse = KHTTPOperationController().WIPPendingDocSubGreaterThan2DaysCustomerInvoiceSearch(customer, invoice)
        if (apiResponse != null) {
